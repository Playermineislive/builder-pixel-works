...
const chatThemes: ChatTheme[] = [
  ...
  },
  {
    ...
    background: 'linear-gradient(135deg, #2c3e50 0%, #4a00e0 50%, #8e2de2 100%)',
  ...
  },
  {
    ...
    background: 'linear-gradient(135deg, #ffeef8 0%, #f8c9d4 50%, #f19cb1 100%)',
  ...
  },
  {
    ...
    background: 'linear-gradient(135deg, #ff9a56 0%, #ffad56 25%, #67b26f 50%, #4ca2cd 100%)',
...
export const ThemeProvider: React.FC<ThemeProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (savedTheme) {
      const theme = [...chatThemes, ...Object.values(weatherThemes)].find(t => t.id === savedTheme);
      if (theme) {
  ...
  const setTheme = (themeId: string) => {
    const theme = [...chatThemes, ...Object.values(weatherThemes)].find(t => t.id === themeId);
    if (theme) {
      setCurrentTheme(theme);
